...
export async function SkateboardProduct({id}: Props) {
  ...
  return (
    <div className='group relative mx-auto w-full max-w-72 px-8 pt-4'>
        ...
        <div className='flex items-center justify-between ~text-sm/2xl text-brand-gray'>
            <span>{price}</span>
            <span className='inline-flex items-center gap-1'>
                <FaStar className='text-yellow-400'/> 24
            </span>
        </div>
        ...
        <HorizontalLine className={HORIZONTAL_LINE_CLASSES} />
        <h3 className='my-2 text-center font-sans leading-tight ~text-lg/xl text-brand-gray'>
            {product.data.name}
        </h3>
